...
async fn main() {
    ...
    let command_path = Config::config_path().with_file_name("command.json");

    loop {
        sleep(Duration::from_secs(2)).await;

        // Check for run-now commands from GUI
        if let Ok(data) = std::fs::read_to_string(&command_path) {
            let _ = std::fs::remove_file(&command_path);
            if let Ok(cmd) = serde_json::from_str::<serde_json::Value>(&data) {
                if let Some(action) = cmd.get("action").and_then(|v| v.as_str()) {
                    match action {
                        "run_task" => {
                            if let Some(id_str) = cmd.get("task_id").and_then(|v| v.as_str()) {
                                if let Ok(task_id) = uuid::Uuid::parse_str(id_str) {
                                    // Reload config immediately so the scheduler has the latest task definition
                                    if let Ok(new_config) = load_config_safe() {
                                        ...
                                        *t = new_tasks.clone();
                                        ...
                                        log_manager.log("Config reloaded for Run Now".to_string());
                                    }
                                    let _ = sched_tx.send(SchedulerCommand::RunTaskNow(task_id)).await;
                                    log_manager.log(format!("Run Now command received for task {}", task_id));
                                }
                            }
                        }
                        _ => {}
